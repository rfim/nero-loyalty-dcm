...
GRANT REFERENCES ON SEMANTIC VIEW NERO_GOVERNANCE.CORTEX_TOOLS.LOYALTY_SEMANTIC_VIEW TO ROLE NERO_COPILOT_ROLE;
-- Semantic views do not run with definer's rights -- the querying role
-- needs its own direct access to the underlying base tables (same as
-- every other Cortex Analyst consumer role, see leader_personas_stack.sql):
GRANT USAGE ON SCHEMA NERO_DB."01_BRONZE" TO ROLE NERO_COPILOT_ROLE;
GRANT SELECT ON ALL TABLES IN SCHEMA NERO_DB."01_BRONZE" TO ROLE NERO_COPILOT_ROLE;
GRANT SELECT ON FUTURE TABLES IN SCHEMA NERO_DB."01_BRONZE" TO ROLE NERO_COPILOT_ROLE;
