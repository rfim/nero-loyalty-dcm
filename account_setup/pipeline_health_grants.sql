...
-- NERO_DB: staging/bronze row counts (renamed from bronze/silver -- see
-- analytics/README.md). No more 02_CONTROL grants -- the audit trail and
-- release pointer PROCESS_BATCH used to write are retired; pipeline
-- health now reads DBT_DAILY_REFRESH_TASK's run history instead
-- (SNOWFLAKE.ACCOUNT_USAGE.TASK_HISTORY, already granted below).
GRANT USAGE ON DATABASE NERO_DB TO ROLE NERO_GOVERNANCE_ROLE;
GRANT USAGE ON SCHEMA NERO_DB."00_STAGING" TO ROLE NERO_GOVERNANCE_ROLE;
GRANT USAGE ON SCHEMA NERO_DB."01_BRONZE" TO ROLE NERO_GOVERNANCE_ROLE;

GRANT SELECT ON ALL TABLES IN SCHEMA NERO_DB."00_STAGING" TO ROLE NERO_GOVERNANCE_ROLE;
GRANT SELECT ON FUTURE TABLES IN SCHEMA NERO_DB."00_STAGING" TO ROLE NERO_GOVERNANCE_ROLE;
GRANT SELECT ON ALL TABLES IN SCHEMA NERO_DB."01_BRONZE" TO ROLE NERO_GOVERNANCE_ROLE;
GRANT SELECT ON FUTURE TABLES IN SCHEMA NERO_DB."01_BRONZE" TO ROLE NERO_GOVERNANCE_ROLE;
