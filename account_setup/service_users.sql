...
CREATE ROLE IF NOT EXISTS NERO_INGEST_ROLE
  COMMENT = 'Ingestion service role: lands typed rows into BRONZE_<DATASET>/BRONZE_BATCH_MANIFESTS and LANDING_STAGE only. Validation/publish into silver is dbt''s job (NERO_DBT_ROLE), not this role''s.';
...
-- ---- NERO_INGEST_ROLE: warehouse + exactly what landing bronze needs ----
-- No more NERO_DB."02_CONTROL" grants -- PROCESS_BATCH/RUN_PENDING retired,
-- validation/publish into silver is dbt's job now (analytics/models/silver/).
...
GRANT SELECT, INSERT ON TABLE NERO_DB."00_BRONZE".BRONZE_STORES TO ROLE NERO_INGEST_ROLE;
GRANT SELECT, INSERT ON TABLE NERO_DB."00_BRONZE".BRONZE_TRANSACTIONS TO ROLE NERO_INGEST_ROLE;
GRANT SELECT, INSERT ON TABLE NERO_DB."00_BRONZE".BRONZE_LOYALTY_EVENTS TO ROLE NERO_INGEST_ROLE;
GRANT SELECT, INSERT ON TABLE NERO_DB."00_BRONZE".BRONZE_LOYALTY_CUSTOMERS TO ROLE NERO_INGEST_ROLE;
GRANT SELECT, INSERT ON TABLE NERO_DB."00_BRONZE".BRONZE_BATCH_MANIFESTS TO ROLE NERO_INGEST_ROLE;
GRANT SELECT ON ALL TABLES IN SCHEMA NERO_DB."01_SILVER" TO ROLE NERO_INGEST_ROLE;
GRANT SELECT ON FUTURE TABLES IN SCHEMA NERO_DB."01_SILVER" TO ROLE NERO_INGEST_ROLE;
