...
CREATE ROLE IF NOT EXISTS NERO_INGEST_ROLE
  COMMENT = 'Ingestion service role: lands typed rows into STAGING_<DATASET>/STAGING_BATCH_MANIFESTS and LANDING_STAGE only. Validation/publish into bronze is dbt''s job (NERO_DBT_ROLE), not this role''s.';
...
CREATE ROLE IF NOT EXISTS NERO_BI_ROLE
  COMMENT = 'Reporting service role: read-only SELECT on bronze data and governance views. Used by Streamlit apps and the future Power BI reader.';
...
CREATE USER IF NOT EXISTS NERO_INGEST_USER
  ...
  DEFAULT_NAMESPACE = 'NERO_DB."00_STAGING"'
...
-- ---- NERO_INGEST_ROLE: warehouse + exactly what landing staging needs ----
-- No more NERO_DB."02_CONTROL" grants -- PROCESS_BATCH/RUN_PENDING retired,
-- validation/publish into bronze is dbt's job now (analytics/models/bronze/).
GRANT USAGE ON WAREHOUSE NERO_LOAD_WH TO ROLE NERO_INGEST_ROLE;
GRANT USAGE ON DATABASE NERO_DB TO ROLE NERO_INGEST_ROLE;
GRANT USAGE ON SCHEMA NERO_DB."00_STAGING" TO ROLE NERO_INGEST_ROLE;
GRANT READ, WRITE ON STAGE NERO_DB."00_STAGING".LANDING_STAGE TO ROLE NERO_INGEST_ROLE;
GRANT SELECT, INSERT ON TABLE NERO_DB."00_STAGING".STAGING_STORES TO ROLE NERO_INGEST_ROLE;
GRANT SELECT, INSERT ON TABLE NERO_DB."00_STAGING".STAGING_TRANSACTIONS TO ROLE NERO_INGEST_ROLE;
GRANT SELECT, INSERT ON TABLE NERO_DB."00_STAGING".STAGING_LOYALTY_EVENTS TO ROLE NERO_INGEST_ROLE;
GRANT SELECT, INSERT ON TABLE NERO_DB."00_STAGING".STAGING_LOYALTY_CUSTOMERS TO ROLE NERO_INGEST_ROLE;
GRANT SELECT, INSERT ON TABLE NERO_DB."00_STAGING".STAGING_BATCH_MANIFESTS TO ROLE NERO_INGEST_ROLE;
GRANT SELECT ON ALL TABLES IN SCHEMA NERO_DB."01_BRONZE" TO ROLE NERO_INGEST_ROLE;
GRANT SELECT ON FUTURE TABLES IN SCHEMA NERO_DB."01_BRONZE" TO ROLE NERO_INGEST_ROLE;
...
GRANT USAGE ON SCHEMA NERO_GOVERNANCE.CORTEX_TOOLS TO ROLE NERO_CI_ROLE;

-- ---- NERO_BI_ROLE: read-only, bronze data + governance views ----
GRANT USAGE ON WAREHOUSE NERO_BI_WH TO ROLE NERO_BI_ROLE;
GRANT USAGE ON DATABASE NERO_DB TO ROLE NERO_BI_ROLE;
GRANT USAGE ON SCHEMA NERO_DB."01_BRONZE" TO ROLE NERO_BI_ROLE;
GRANT SELECT ON ALL TABLES IN SCHEMA NERO_DB."01_BRONZE" TO ROLE NERO_BI_ROLE;
GRANT SELECT ON FUTURE TABLES IN SCHEMA NERO_DB."01_BRONZE" TO ROLE NERO_BI_ROLE;
