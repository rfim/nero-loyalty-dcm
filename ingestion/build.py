...
DCM SQL + reference SQL, and (with --seed) loads the contract into
CONTROL_DATA_CONTRACTS.

Reads the contract (business rules + ingestion strategy + PII tags, split
one YAML per dataset — see ingestion/contract_loader.py) and emits:
...
def seed(connection, doc, contract_hash):
...
def main():
    ...
    if args.seed:
        seed(args.connection, doc, contract_hash)
