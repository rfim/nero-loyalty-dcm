...
-- validated_stores when present (a null FK is valid, not a violation).
select
    c.customer_id,
    c.signup_date,
    c.tier,
    ...
    c.batch_id,
    c.ingested_at
from {{ source('nero_bronze', 'loyalty_customers') }} c
where c.customer_id is not null
  and c.signup_date is not null
  and c.tier in ('Bronze', 'Silver', 'Gold')
  and (
    c.home_store_id is null
    or exists (select 1 from {{ ref('validated_stores') }} s where s.store_id = c.home_store_id)
  )
