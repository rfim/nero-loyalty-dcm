-- Staging -> bronze, replacing PROCESS_BATCH's Python validation with plain
-- SQL filtering: only rows meeting every contract rule reach this table.
-- Staging is append-only across every historical batch (each full-mode
...
select
    ...
    batch_id,
    ingested_at
from {{ source('nero_staging', 'stores') }}
