...
-- only staging rows newer than what's already here (is_incremental()
-- filter against this table's own max ingested_at), MERGEd in by
...
select
    ...
    t.batch_id,
    t.ingested_at
from {{ source('nero_staging', 'transactions') }} t
where t.transaction_id is not null
  ...
  and exists (select 1 from {{ ref('bronze_stores') }} s where s.store_id = t.store_id)
  and (
    t.customer_id is null
    or exists (select 1 from {{ ref('bronze_loyalty_customers') }} c where c.customer_id = t.customer_id)
  )
{% if is_incremental() %}
